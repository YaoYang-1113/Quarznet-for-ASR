...
class AudioGenerator():
    def __init__(self, step=10, window=20, max_freq=8000, mfcc_dim=13,
        minibatch_size=20, desc_file=None, spectrogram=True, max_duration=30.0,
    ...
    def get_batch(self, partition):
        ...
        if partition == 'train':
            ...
            cur_index = self.cur_train_index           #random.randint(0,len(self.train_texts) - mini_batchsize)
            texts = self.train_texts
        elif partition == 'valid':
            audio_paths = self.valid_audio_paths
            cur_index = self.cur_valid_index           #random.randint(0,len(self.valid_texts) - mini_batchsize)
...
def plot_spectrogram_feature(index):
    audio_gen = AudioGenerator(spectrogram=True)
    audio_gen.load_train_data()
    vis_audio_path = audio_gen.train_audio_paths[index]
    vis_spectrogram_feature = audio_gen.normalize(audio_gen.featurize(vis_audio_path))
